...
Процедура ДополнитьФормуИнтеграциейДиадок(Форма, ИмяТаблицыДокументов = "") Экспорт
	...
	Если Форма.Объекты.Найти("СтатусЭДО") = Неопределено Тогда
	...
	КомандаПолучитьСтатус.Картинка = БиблиотекаКартинок.Пересчитать;
	...
	КомандаПерейтиВЛК.Картинка = БиблиотекаКартинок.Гиперссылка;
	
	Если ЗначениеЗаполнено(ИмяТаблицыДокументов) Тогда
		КомандаПолучитьСтатус.РежимОбработки = РежимОбработкиКомандыФормы.Множественный;
		КомандаПолучитьСтатус.Источник.Имя = ИмяТаблицыДокументов;
		КомандаПерейтиВЛК.РежимОбработки = РежимОбработкиКомандыФормы.Одиночный;
		КомандаПерейтиВЛК.Источник.Имя = ИмяТаблицыДокументов;
	КонецЕсли;
	...
	// 4. Для формы списка добавляем поле статуса в таблицу
	Если ЗначениеЗаполнено(ИмяТаблицыДокументов) Тогда
		
		Попытка
			ТабличнаяЧасть = Форма.Элементы.Найти(ИмяТаблицыДокументов);
			
			Если ТабличнаяЧасть <> Неопределено Тогда
				
				ПолеСтатусДиадок = Форма.Элементы.Добавить("ПолеСтатусЭДО", Тип("ПолеФормы"), ТабличнаяЧасть);
				ПолеСтатусДиадок.Вид = ВидПоляФормы.ПолеНадписи;
				ПолеСтатусДиадок.ПутьКДанным = ИмяТаблицыДокументов + ".СтатусЭДО";
				ПолеСтатусДиадок.Заголовок = НСтр("ru = 'Статус (Диадок)'");
				
			КонецЕсли;
		Исключение
			// Если не удалось добавить поле - не критично, продолжаем работу
			// Возможно, таблица не найдена или недостаточно прав
		КонецПопытки;
