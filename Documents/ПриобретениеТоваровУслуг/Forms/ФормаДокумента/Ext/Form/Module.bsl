...
Функция Перечисление_СтатусыДокументовАПИ()
		...
		Результат.Вставить("InvoiceAmendmentRequested", "Ожидается уточнение");
		Результат.Вставить("RecipientSignatureRequestRejected", "Отказано в подписи контрагенту");
		Результат.Вставить("WithRecipientSignature", "Подписан");
		Результат.Вставить("WaitingForRecipientSignature", "Требуется подпись");
		Результат.Вставить(	"RecipientSignatureRequestRejected", "Контрагент отказал в подписи");
		Результат.Вставить("WaitingForReceipt", "Ожидается извещение о получении");
		Результат.Вставить(	"WaitingForRecipientSignature", "Ожидается подпись контрагента");
...
&НаКлиенте
Процедура ПолучитьСтатусДиадокЗавершение(Документ) 
...
Процедура ДобавитьЗаписиВРС(Статус, Документ) 
	...
	//<< Портал-Юг, Кондрашихин, 2023.10.23 [Обновление ERP 2.5] // Описание:
	// Если доп реквизитов по идентификаторам для формул не нашли, в РС ничего не пишем.
	//
	...
	Если СвДата = ДопРекиСведПустаяСсылка ИЛИ СвСтатус = ДопРекиСведПустаяСсылка Тогда 
		Возврат;
	КонецЕсли;
	//>> Портал-Юг, Кондрашихин, 2023.10.23
